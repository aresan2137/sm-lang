...
pub fn make_cpp(ast: &AST) -> String {
    ...
    for func in &ast.functions {
        ...
        process_part(&func.body, &mut code);
...
fn process_part(part: &Part, code: &mut String) {
    match part {
        Part::Parts { parts } => {
            code.push_str("{\n");
            for part in parts {
                process_part(part, code);
            }
            code.push_str("}\n");
        }
